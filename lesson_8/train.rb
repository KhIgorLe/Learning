...
class Train
  ...
  def validate!
    raise 'Вы не задали номер поезда' if @number.to_s.empty?
    raise 'Вы не задали тип поезда' if @type.to_s.empty?
    raise 'Номер поезда имеет неверный формат' if @number.to_s !~ NUMBER_FORMAT
  end
end
